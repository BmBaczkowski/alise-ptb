function generalisation(varargin)
...
load(fullfile(resourcesdir, 'pseudorandTrials4generalisation.mat'), 'pseudorandTrials');
...
shocks = trials_conditions==0;
graph = ismember(trials_conditions, [0 1 2 3 4]);
graph = graph + 2*ismember(trials_conditions, [5 6 7 8]);
selectednodes = [1 5 10 13];
nodes = selectednodes(1)*ismember(trials_conditions, [0 1 5]);
nodes = nodes + selectednodes(2)*ismember(trials_conditions, [2 6]);
nodes = nodes + selectednodes(3)*ismember(trials_conditions, [3 7]);
nodes = nodes + selectednodes(4)*ismember(trials_conditions, [4 8]);
markers = trials_conditions*10;
% mark the shock
markers = markers + (trials_conditions==0)*90;
conditions = trials_conditions;
% put back the CS+
...
pictures = nan(size(conditions,1),1);
for i = 1:size(conditions,1)
    
    pictures(i) = pics2nodes(nodes(i),graph(i));
    
end

pics2textures = nan(size(selectednodes,2)*2,1);
counter = 1;
for g = 1:2
    for n = 1:size(selectednodes,2)
        pics2textures(counter) = pics2nodes(selectednodes(n), g);
        counter = counter + 1;
    end
end

% design matrix
X = [(1:size(conditions,1))' ...
    pictures ...
    nodes ...
    graph ...
    conditions ...
...
% load images
stimTex = nan(size(pics2textures,1),1);
for i = 1:size(pics2textures,1)
    pic           = pics2textures(i);
...
for trial = 1:nTrials
    ...
    % Draw stimulus
    Screen('DrawTexture', window, stimTex( X(trial,5) )); % texture is the same as conditions
    % Draw fixation dot
    Screen('DrawDots', window, [dotXpos dotYpos], dotSizePix, dotColor, [], 2);
    ...
    fprintf(logfile, '%s,', ...
        num2str(X(trial,1)), ... % trial
        num2str(X(trial,2)), ... % picture
        num2str(X(trial,3)), ... % node
        num2str(X(trial,4)), ... % graph
        num2str(X(trial,5)), ... % conditions
        num2str(X(trial,6)), ... % shock 
        num2str(X(trial,7)), ... % marker
        num2str(response), ...
        sprintf('%0.3f', RT));
    fprintf(logfile, '\n');
    fclose(logfile);
